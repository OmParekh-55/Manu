...
class AuthService {
  ...
  async createBusinessProfile(input: { businessName: string; businessType: BusinessType; ownerName: string; email: string; phone: string }): Promise<{ success: boolean; business?: Business; message?: string }> {
  ...
  async finalizeBusinessSecurity(businessId: string, ownerPassword: string, staffPassword: string): Promise<{ success: boolean; user?: AuthUser; tokens?: AuthTokens; message: string }> {
    try {
      const { businessRepository } = await import('@/services/indexeddb/repositories/businessRepository');
      ...
      notificationService.success('Business Profile Created', `Your Business Code is ${business.businessCode}. Share it with staff to join.`);
